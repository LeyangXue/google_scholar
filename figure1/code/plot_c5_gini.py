#!/usr/bin/env python3
"""Plot yearly C5 citation Gini coefficients for MAG and OpenAlex."""
...
def add_google_scholar_marker(ax, text_height=0.85):
...
def add_digitization_marker(ax, text_height=0.64):
...
def plot_group(
    ...
    ax.set_title(display_titles[group], fontsize=9, pad=5)
...
def plot_dataset_row(fig, outer_spec, data, dataset_name, colors, top_row):
    ...
    plot_group(
        ...
        show_xlabel=True,
        show_ylabel=True,
        end_year=top_one_last_year - 5,
    )
    add_digitization_marker(left_ax)
    marker_height = 0.86 if dataset_name == "OpenAlex" else 0.85
    add_google_scholar_marker(left_ax, text_height=marker_height)

    right_grid = GridSpecFromSubplotSpec(
        3, 1, subplot_spec=outer_spec[1], hspace=0.38
    )
    small_axes = []
...
def make_figure(mag_data, openalex_data, output_path, colors):
    ...
    add_panel_labels(
        ...
        ["(a)  MAG", "(b)  MAG", "(c)  OpenAlex", "(d)  OpenAlex"],
    )
    add_era_legend(fig)
    os.makedirs(output_path, exist_ok=True)
    fig.savefig(os.path.join(output_path, "Figure1_gini_C5.png"), dpi=600)
    fig.savefig(os.path.join(output_path, "Figure1_gini_C5.pdf"))


if __name__ == "__main__":

    mag_file = "/Volumes/lydisk/work/work11/google_scholar/figure1/result/gini/c5_gini_mag.csv"
    openalex_file = "/Volumes/lydisk/work/work11/google_scholar/figure1/result/gini/c5_gini_openalex.csv"
    output_path = "/Volumes/lydisk/work/work11/google_scholar/figure1/figure"
