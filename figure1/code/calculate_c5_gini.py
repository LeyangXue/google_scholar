#!/usr/bin/env python3
"""Calculate yearly C5 citation Gini coefficients by citation percentile group."""
...
def calculate_dataset_gini(data_path, dataset_name, years, percentile_groups):
    """Load annual C5 data and calculate Gini coefficients for all groups."""
    rows = []
    ...
    for year in years:
        ...
        citation_data = pd.read_csv(input_file, usecols=["C5"])
        citations = pd.to_numeric(citation_data["C5"], errors="coerce").dropna().to_numpy()
        ...
        for group_name, bounds in percentile_groups.items():
            ...
            rows.append({
                ...
                "Gini_C5": gini_coefficient(group_values),
            })
...
def save_results(results, output_path, dataset_name):
    ...
    output_file = os.path.join(output_path, f"c5_gini_{dataset_name.lower()}.csv")
...
if __name__ == "__main__":
    ...
    output_path = "/Volumes/lydisk/work/work11/google_scholar/figure1/result/gini"
    ...
    combined_file = os.path.join(output_path, "c5_gini_combined.csv")
